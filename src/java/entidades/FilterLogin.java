...
@WebFilter(filterName = "FilterLogin", urlPatterns = {"/privado/*"})
public class FilterLogin implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        try {
            ...
            Cookie[] laCookie = pedido.getCookies();

            int posicion = -1;
            
            
            for (int i = 0; laCookie != null && i < laCookie.length; i++) {
                if (laCookie[i].getName().equals("loginSystem")){
                    posicion = i;
                }
            }
            
            if ( laCookie != null && laCookie.length > 0 && posicion >= 0){
                System.out.println("Dale no mas");
