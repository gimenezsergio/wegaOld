class Busqueda {
    ...
    static main() {
        document.querySelector("#consutarBusquedaIngresos").setAttribute("onclick", "Busqueda.producto();");
    }
    static producto() {
        SpinnerModal.open();
        const traer = async () => {
            ...
            let busProv = {};
            busProv.id_producto = document.querySelector("#producto_id").value;
            ...
            if (busProv.fecha_desde === "" || busProv.fecha_hasta === "") {
                ...
                let respuesta = await fetch("../../BusquedaIngresosProducto?&busqueda=" + registroStringJSON,
                    { method: 'GET' });
                ...
                if (no_oks === undefined || no_oks.length == 0) {
                    ...
                    <thead>            
                        <tr>            
                        <th>Proveedor</th><th>Cantidad</th>
                        </tr>
                    </thead>
                    ${ no_oks.map(no_ok =>
                        ` 
                    <tr class='persona_item'>
                        <td>${no_ok.proveedor}</td>
